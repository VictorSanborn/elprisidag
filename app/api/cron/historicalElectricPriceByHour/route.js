...
export async function GET(req) {
	...
	const dayToSearch = searchParams.get('date');

	if (!EIC) {
		return NextResponse.json({ error: true, message: 'EIC is required' });
	}
	if (!dayToSearch) {
		return NextResponse.json({ error: true, message: 'date is required' });
	}
	...
	//Add days
	let startDate = add(dayToSearch, 0); //today or tomorrow based on the query
